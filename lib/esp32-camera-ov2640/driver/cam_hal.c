 ...
 * mystery. Combined-build capture measured: 113-255us = 5-11 LINES, varying
 ...
 * Field report: "sometimes 1 point shifts, sometimes 2, sometimes all
...
/* NOTE: BOTH of the gates above are patch-only additions. The stock driver
 ...
 *
 * REVERTED TO DEFAULT ON after field data caught the off-default in one test
 * ("capture is incorrect ... points duplicated on the Y axis"). The reasoning that produced it — "the lab does not have these, so
 ...
 * the faults persist, the theory is wrong -- look elsewhere. */
