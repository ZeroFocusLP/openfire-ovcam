...
#include "xclk.h"
/* LIGHTGUN: S3+OV2640-only fork. sdkconfig enables every sensor and -U
 * flags cannot beat an in-header #define; without this the linker pulls
 * the other sensors from the core's PREBUILT esp32-camera archive
 * (mixed-version objects). Must sit BEFORE the conditional includes. */
...
#undef CONFIG_HM0360_SUPPORT
#undef CONFIG_HM1055_SUPPORT
#undef CONFIG_MEGA_CCM_SUPPORT
