...
#include "esp_timer.h"      /* LIGHTGUN v9.1: ISR VSYNC period */
#include "ll_cam.h"
#include "cam_hal.h"
...
/* LIGHTGUN v9.1: VSYNC period measured IN THE ISR — immune to cam_task
 * scheduling jitter. The frame gate in the app reads this instead of its own
 * task-context measurement (which is polluted by event-queue latency, the
 * very thing that spikes during the anomalies the gate must catch). */
volatile uint32_t cam_patch_vsync_isr_period_us = 0;
/* v15.3: free-running VSYNC count. Everything else that counts VSYNCs is a
 * windowed accumulator somebody resets; the frame-accounting arithmetic needs
 * a denominator that nothing else can zero underneath it. */
volatile uint32_t cam_patch_vs_total = 0;
static int64_t s_lightgun_vs_prev_us = 0;
...
static void CAMERA_ISR_IRAM_ATTR ll_cam_vsync_isr(void *arg)
{
    ...
    if (status.cam_vsync_int_st) {
        int64_t now = esp_timer_get_time();          /* IRAM-safe */
        cam_patch_vs_total++;                        /* v15.3 */
        if (s_lightgun_vs_prev_us)
            cam_patch_vsync_isr_period_us = (uint32_t)(now - s_lightgun_vs_prev_us);
        s_lightgun_vs_prev_us = now;
...
/* ===================== LIGHTGUN PATCH: EXACT XCLK =====================
 * Stock (both call sites) did:
 *     cam_clkm_div_num = 160000000 / xclk_freq_hz;   // INTEGER division
 *     cam_clkm_div_a = cam_clkm_div_b = 0;           // fraction disabled
 * so the S3 could only emit 160/N MHz. Requesting 27MHz silently produced
 * 160/5 = 32MHz -> 64MHz sysclk through the OV2640's 2x PLL, ~33% past its
 * analog limit: field-verified dead, banded frames where bright light reads
 * DARK, plus a bogus "160fps" (= 2.5 x 64, our own fps law).
 *
 * The LCD_CAM divider is really (N + B/A) with 6-bit A/B, so the frequencies
 * in between ARE reachable. 27MHz: N=5, B/A ~ 0.926 -> 160/(5+0.926) = 27.0.
 * This restores on S3 the exact clock the classic ESP32 made with LEDC — the
 * one that produced working 135fps capture.
 *
 * Exports the frequency actually programmed so firmware can print truth
 * instead of intent (ledc_get_freq() is meaningless here: the S3 camera clock
 * does not come from LEDC at all).
 */
uint32_t cam_patch_actual_xclk_hz = 0;

void lightgun_set_cam_clk(uint32_t xclk_freq_hz)
{
    const uint32_t src = 160000000u;
    if (xclk_freq_hz < 1000000u) xclk_freq_hz = 1000000u;
    uint32_t n = src / xclk_freq_hz;
    if (n < 2) n = 2;                       /* hardware minimum */
    if (n > 255) n = 255;
    uint32_t a = 0, b = 0;
    uint32_t rem = src - n * xclk_freq_hz;  /* 0 <= rem < xclk_freq_hz */
    if (rem > 0) {
        a = 63;                             /* max denominator = best precision */
        b = (uint32_t)(((uint64_t)a * rem + xclk_freq_hz / 2) / xclk_freq_hz);
        if (b == 0)      { a = 0; }                 /* rounds down to integer N */
        else if (b >= a) { a = 0; b = 0; n += 1; }  /* rounds up to N+1 */
    }
    LCD_CAM.cam_ctrl.cam_clkm_div_b = b;
    LCD_CAM.cam_ctrl.cam_clkm_div_a = a;
    LCD_CAM.cam_ctrl.cam_clkm_div_num = n;
    LCD_CAM.cam_ctrl.cam_clk_sel = 3;       /* PLL160 (empirically: 20MHz req -> 20MHz out) */
    cam_patch_actual_xclk_hz = (a && b)
        ? (uint32_t)(((uint64_t)src * a) / ((uint64_t)n * a + b))
        : src / n;
}
/* =================== END LIGHTGUN PATCH: EXACT XCLK =================== */
...
esp_err_t ll_cam_config(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    lightgun_set_cam_clk(config->xclk_freq_hz);   /* LIGHTGUN: exact fractional XCLK */
...
static bool ll_cam_calc_rgb_dma(cam_obj_t *cam){
    ...
    /* LIGHTGUN v15.1 — THE OVERRUN BUDGET, made tunable.
     * This is the only knob that sets how much slack cam_task has before the
     * DMA laps the ring and splices a frame. Harness measurements
     * measured every anomalous point displacement as an EXACT multiple of 4 or
     * 8 half-buffers (worst deviation 0.6 of a line over 25 events) — i.e. the
     * fault IS a ring lap, and this number is its budget. Default is the IDF
     * config value; -D LIGHTGUN_DMA_RING_BYTES=65536 doubles it for ~30KB of
     * extra DRAM. Raising it does not fix whatever blocks core 1 — it buys
     * time so that blocking stops corrupting frames. */
#ifdef LIGHTGUN_DMA_RING_BYTES
    size_t dma_half_buffer_max = LIGHTGUN_DMA_RING_BYTES / 2 / cam->dma_bytes_per_item;
#else
...
size_t IRAM_ATTR ll_cam_memcpy(cam_obj_t *cam, uint8_t *out, const uint8_t *in, size_t len)
{
    ...
    if (cam->in_bytes_per_pixel == 2 && cam->fb_bytes_per_pixel == 1) {
        ...
        /* LIGHTGUN: stock returned len/2 unconditionally while the loop only
         * ever wrote (len/8)*4 bytes. For any len that is not a multiple of 8
         * the frame's write cursor advanced PAST bytes that were never written:
         * a gap of stale buffer, and every row after it offset by a constant --
         * i.e. the image jumps sideways and steps down, with no per-line resync
         * to recover (this path fills the framebuffer as one linear stream).
         * Handle the tail and report what was actually written.
         * NOTE: when len % 8 == 0 -- every current configuration -- this is
         * bit-identical to the stock code. */
        size_t written = end * 4;
        size_t rem = len - end * 8;          /* 0..7 leftover wire bytes */
        for (size_t i = 0; i < rem; i += 2) {
            *out++ = in[i];                  /* luma sits on even offsets */
            written++;
        }
        return written;
    }
